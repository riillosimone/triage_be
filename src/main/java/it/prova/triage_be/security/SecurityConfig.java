package it.prova.triage_be.security;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable() // Disabling csrf
				...
				.cors() // Enabling cors
				.and()
				.authorizeHttpRequests().antMatchers("/api/auth/login", "/h2-console/**").permitAll()
				.antMatchers("/swagger-ui/**", "/v3/api-docs/**").permitAll()
				.antMatchers("/api/tavolo/**").permitAll()
				// tutti gli utenti autenticati possono richiedere le info
				.antMatchers("/api/utente/userInfo").authenticated()
				.antMatchers("/api/dottore/**").authenticated()
				.antMatchers("/api/utente/**","/api/protected/**").hasRole("ADMIN")
